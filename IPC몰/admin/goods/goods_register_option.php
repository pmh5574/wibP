<script type="text/javascript" xmlns="http://www.w3.org/1999/html">
    ...
    function fill_option_layer() {
        ...
        // 디자인위브 커스텀 21_05_24
        $wibType = explode("^|^", $data['wibType']);
        $colorOptionsData = explode("^|^", $data['colorOptions']);
        $colorOptionArray = [];
        foreach ($colorOptionsData as $value) {
            if($value){
                $exVal = explode("||", $value);
                $colorOptionArray[$exVal[0]] = $exVal[1];
            }
        }
        echo "var _colors = ".json_encode($colorOptionArray).";";
        // 디자인위브 커스텀 끝 21_05_24
        
        for ($i = 0; $i < $data['optionCnt']; $i++) {
            $optionCnt = count(gd_isset($data['option']['optVal'][$i + 1]));
            // 디자인위브 커스텀 21_05_24
            echo "$('input:radio[name=\'optionY[optionWibtype][".$i."][]\']:input[value=\'".$wibType[$i]."\']').prop('checked', true);". chr(10);
            // 디자인위브 커스텀 끝 21_05_24
            ...
            if (is_array($data['option']['optVal'][$i + 1])) {
                ...
                foreach ($data['option']['optVal'][$i + 1] as $key => $val) {
                    ...
                    if($wibType[$i] == 'c'){
                        ?>
                        var _optTarget = $('#option_optionValue_<?=$i?>_<?=$j?>');
                        _optTarget.addClass('wibPickerOption');
                        _optTarget.next('div').addClass('wibPickerBox');
                        _optTarget.prop('placeholder', '색상값을 입력해 주세요. ex) 파랑 ');
                        _optTarget.next('div').find('.color-selector').val(_colors['<?=$val?>']);
                        _optTarget.next('div').find('.color-selector-btn').css('background-color', _colors['<?=$val?>']);
                        <?php
                    }
                    // 디자인위브 커스텀 끝 21_05_24
            ...
            //echo "  $(':radio[name=\"optionY[optionWibtype][" . $i . "][]\"][value=\"" . $wibType[$i] . "\"]').prop('checked', 'checked');" . chr(10);
        }
    ...
    function option_setting_layer(thisCnt) {
        ...
        if (fieldCnt == '0' && fieldChk > 0) {
            ...
            templateHtml += '<tr>';
            // 디자인위브 커스텀 21_05_20
            templateHtml += '<th class="left width-md">구분</th>';
            // 디자인위브 커스텀 끝 21_05_20
        ...
        if (fieldChk > 0) {
            ...
            for (var i = fieldCnt; i < thisCnt; i++) {
                ...
                // 디자인위브 커스텀 21_05_20
                addHtml += '<td><label class="radio-inline">';
                addHtml += '<input type="radio" name="optionY[optionWibtype]['+i+'][]" value="n" onclick="optionTypeChange(\'n\', '+i+')" checked>일반</label>';
                addHtml += '<label class="radio-inline">';
                addHtml += '<input type="radio" name="optionY[optionWibtype]['+i+'][]" value="c" onclick="optionTypeChange(\'c\', '+i+')">색상</label></td>';
                // 디자인위브 커스텀 끝 21_05_20
                ...
                addHtml += '<div style="display:none"><input type="text" class="color-selector" ></div>';
    ...
    function option_value_conf_layer(loc, thisCnt, loadChk) {
        ...
        if (fieldChk > 0) {
            ...
            for (var i = fieldCnt; i < thisCnt; i++) {
                ...
                // 구분값 일반/색상에 따른 레이아웃 변경
                var _locVal = $('input[name="optionY[optionWibtype][' + loc + '][]"]:checked').val();
                if(_locVal == 'n'){
                    ...
                    addHtml += '<input type="text" id="option_optionValue_' + loc + '_' + i + '" data-option-sno="'+loc+'" name="optionY[optionValue][' + loc + '][]" value="" class="form-control wibPickerOption" style="width:330px;" placeholder="색상값을 입력해 주세요. ex) 파랑" onblur=" if(option_value_check_layer(\'' + loc + '\',\'' + i + '\') == true) { option_grid_layer(); } " maxlength="255"/>';
                    addHtml += '<div class="wibPickerBox" style="display:none"><input type="text" class="color-selector" ></div>';
                }
    ...
    function option_grid_layer(manualFl) {
        ...
        // 색상 선택 되었는지 확인
        var typePass = true;
        for(i=0;i<$('[id*="option_optionName_layer_"]').length;i++){
            
            var _opWibtype = $('input[name="optionY[optionWibtype]['+i+'][]"]:checked').val();
            
            if(_opWibtype == 'c'){
                $('#optionValueLayer'+i+' .color-selector').each(function(){
                    if($(this).val() == ''){
                        typePass = false;
                        return false;
                    }
        ...
        for (var j = 0; j <= optTotCnt; j++) {
            if (j == 0) {
                ...
                for (var k = 0; k < optionCnt; k++) {
                    ...
                    _wibType += wibTypeChain + $('input[name="optionY[optionWibtype]['+k+'][]"]:checked').val();
                    
                }
                addHtml += '<input type="hidden" name="wibType" value="'+_wibType+'">';
                // 디자인위브 커스텀 끝 21_05_24
                ...
                for (var k = 0; k < optionCnt; k++) {
                    ...
                    if($('#option_optionValue_' + k + '_' + valIdNo[k]).length) {
                        ...
                        // 디자인위브 커스텀 21_05_24
                        var _colorVal = $('#option_optionValue_' + k + '_' + valIdNo[k]).next('div').find('.color-selector').val();
                        console.log(_colorVal);
                        var _newOptVal = optVal;
                        if(_colorVal != ''){
                            _newOptVal += '<span style="background-color: '+_colorVal+';width: 18px !important;height: 18px !important;display: inline-block;vertical-align: bottom;margin-left: 5px;"></span>';
                            arrColors.push(optVal+'||'+_colorVal);
                        }
                        addHtml += '<td class="center '+optClass+'">' + _newOptVal + '</td>';
                        // 디자인위브 커스텀 끝 21_05_24
        ...
        // 디자인위브 커스텀 21_05_24
        var final_data = [];
        $.each(arrColors,function(i,value){
            if(final_data.indexOf(value) == -1 ) final_data.push(value);
        });

        var finColors = final_data.join("^|^"); 
        addHtml += '<input type="hidden" name="colorOptions" value="'+finColors+'">';
        // 디자인위브 커스텀 끝 21_05_24
    ...
    // 디자인위브 커스텀 21_05_20
    // 옵션 구분값에 따른 옵션 레이아웃 변형
    function optionTypeChange(type, num){ 
        var _target = $(":text[name='optionY[optionValue][" + num + "][]']");
        if(type == 'n'){
            _target.removeClass('wibPickerOption');
            _target.next('div').removeClass('wibPickerBox');
            _target.prop('placeholder', 'Enter키를 이용 옵션값을 연속적으로 입력하세요. ex)XL')
        }else if(type == 'c'){
            _target.addClass('wibPickerOption');
            _target.next('div').addClass('wibPickerBox');
            _target.prop('placeholder', '색상값을 입력해 주세요. ex) 파랑 ');
        }
        
    }
    
    // 컬러피커 초기화
    function init_color_picker_new() {
...
<style>
.wibPickerOption {width: 230px !important;}
.wibPickerBox {display: inline-block !important;}
.wibPickerBox .color-selector {width: 70px;text-align: center;height: 23px;margin-left: 5px}
.wibPickerBox .color-selector-btn {width: 23px !important;height: 23px !important;}
</style>
                        ...
                        <tr id="tbl_option_info_<?=$key+1?>">
                            ...
                            for ($i = 0; $i < $data['optionCnt']; $i++) {
                                ...
                                <td class="center <?=$optClass?>">
                                    ...
                                    <?php if($colorOptionArray[$data['option'][$key]['optionValue' . ($i + 1)]]){ 
                                        echo '<span style="background-color: '.$colorOptionArray[$data['option'][$key]['optionValue' . ($i + 1)]].';width: 18px !important;height: 18px !important;display: inline-block;vertical-align: bottom;margin-left: 5px;"></span>';
                                    } ?>
                                </td>
                                <?php
                            }
