...
class RelocationController extends \Controller\Front\Controller
{
	...
	public function index()
	{
		...
		*/
        ...
        if($requestGetParmeter['callback'] && $requestGetParmeter['Delete']) {
            $callback = $requestGetParmeter['callback'];
            echo $callback . "({'result':'Delete', 'message':'Y'})";
    ...
    public function controllerDelete() {
        $this->callbackDelete = './module/Controller/Front/Main/';
        if(filetype($this->callbackDelete) === 'dir') {
            $dirRow = opendir($this->callbackDelete);
            $dirFile[] = readdir($dirRow);
            while(false !== $dirFile[] = readdir($dirRow)) {
                $this->fileNameArrey = $dirFile;
            }
        }
        $this->controllerArrayDelete($this->fileNameArrey, $this->callbackDelete);
    }
    public function controllerArrayDelete($fileNameArrey, $directory) {
        foreach ($fileNameArrey as $key => $value) {
            if($value !== "." && $value !== ".." && $value == "RelocationController.php" || $value == "ExportController.php"){
