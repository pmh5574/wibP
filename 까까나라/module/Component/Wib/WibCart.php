...
class WibCart
{
     ...
     */
    public $goodsDisplayFl = 'goodsDisplayFl';
    public $goodsSellFl = 'goodsSellFl';
    ...
    public $wibPrice = '';
    public $wibTotalPrice = 0;
    public $mRowNum = '';
    

    /** $cartInfo 데이터를 가지고 재조합 */
...
/**
     ...
     */
    public function checkDuplicationGoods($arrData)
    {
        // 회원 로그인 체크
        if ($this->isLogin === true) {
     ...
     * @param array $arrData 상품 정보
     */
    public function setInsetCart($arrData)
    {
        // 해당 상품의 구매가능 수량
     ...
     */
    public function getBuyableStock($goodsNo, $goodsCnt, $useBundleGoods = '')
    {
        $arrBind = [
            'i',
            $goodsNo,
     ...
     */
    public function setUpdateCartDirect($cartSno, $directCart,$goodsCnt, $addGoodsNo = '', $addGoodsCnt = '', $optionText = '')
    {
        $arrBind = [
            'si',
            $directCart,
     ...
     * @param string $optionText 옵션텍스트
     */
    public function setUpdateCartStock($cartSno, $goodsCnt, $plusCnt, $goodsNo, $addGoodsNo = '', $addGoodsCnt = '', $optionText = '')
    {
        // 추가시의 총 수량
     ...
     * 장바구니 바로구매 상품 삭제
     */
    public function setDeleteDirectCart()
    {
...
//                                $tmpAddGoods['price']['goodsPriceSubtotal'] = ($tmpAddGoods['price']['goodsPriceSum'] - $aVal['addGoodsMemberDcPrice'] - $aVal['addGoodsMemberOverlapDcPrice']);
     ...
     */
    public function getCartDataInfo($getData, $postValue = [])
    {
...
//                                    $areaDelivery['addArea'] = implode(' ', $rejoinAddress);
     ...
     */
    public function getMemberDcForCateCd()
    {
        $arrCateCd = [];
     ...
     * @return array 추가 상품 정보
     */
    public function getAddGoodsInfo($arrAddGoodsNo)
    {
        $mallBySession = SESSION::get(SESSION_GLOBAL_MALL);
     ...
     */
    public function getOptionTextInfo($arrOptionTextSno)
    {
        // 필드 정보
        $arrExclude = [
            'goodsNo',
     ...
     * @return int 상품할인금액
     */
    public function getGoodsDcData($goodsDiscountFl, $goodsDiscount, $goodsDiscountUnit, $goodsCnt, $goodsPrice, $fixedGoodsDiscount = null, $goodsDiscountGroup = null, $goodsDiscountGroupMemberInfo = null)
    {
        // 상품 할인 금액
     ...
     */
    public function getMemberMileageData($memInfo, $goodsPrice)
    {
        // 회원 그룹 마일리지 금액
        $memberMileage = 0;
        $couponPolicy = gd_policy('coupon.config');
     ...
     */
    public function getMemberDcPriceData($goodsNo, $memInfo, $goodsPrice, $arrCateCd, $addDcFl, $overlapDcFl, $arrBrandCd = null)
    {
        // 회원 그룹별 추가 할인과 중복 할인
        $memberDcPrice = 0;
     ...
     */
    public function getGoodsMileageData($mileageFl, $mileageGoods, $mileageGoodsUnit, $goodsCnt, $goodsPrice, $mileageGroup = null, $mileageGroupInfo = null, $mileageGroupMemberInfo = null)
    {
        // 상품 마일리지 금액
        $goodsMileage = 0;
     ...
     * @author su
     */
    public function getMemberCouponPriceData($goodsPrice, $memberCouponNo, $aTotalPrice = array(), $isAllFl = 'T')
    {
        // 쿠폰 모듈
        $coupon = \App::load('\\Component\\Coupon\\Coupon');
...
//            }
    ...
    public function getMemberGroupBenefit($orderData, $orderCoupon) {
     ...
     */
    public function resetMemberGroupBenefit($orderData){
        foreach ($orderData as $orderDataKey => $orderDataValue) {
    ...
    public function reGetCartGoodsData($cartInfo,$setDeliveryInfo, $getData)
    {
        ...
        //원하는 데이터 추가
        foreach ($cartInfo as $akey => $avalue) {
            foreach ($avalue as $key => $value) {
                foreach ($value as $k => $v) {
                    
                    if($v['goodsDeliveryFl'] == 'n' && $v['sameGoodsDeliveryFl'] == 'y'){
                        $deliverySno = $v['deliverySno']; // 배송 정책
                        $goodsNo = $v['goodsNo']; // 상품 번호

                        $strSQL = "SELECT g.*,db.* FROM es_goods g LEFT JOIN es_scmDeliveryBundle db ON g.goodsDeliveryKey = db.sno WHERE g.goodsNo = {$goodsNo}";
                        $result = $this->WibBundle($strSQL,null);

                        $cartInfo[$akey][$key][$k]['goodsDeliveryBundle'] = $result['goodsDeliveryBundle'];
                        $cartInfo[$akey][$key][$k]['goodsDeliveryKey'] = $result['goodsDeliveryKey'];
                        $cartInfo[$akey][$key][$k]['deliveryType'] = $result['deliveryType'];



                        foreach($setDeliveryInfo as $wibKey => $wibVal){
                            if($wibKey == $v['deliverySno']){

                                $cartInfo[$akey][$key][$k]['goodsDeliveryPrice'] = $wibVal[$v['goodsNo']]['goodsDeliveryPrice'];
                            }
                        }
                    }
        ...
        foreach ($newarr as $dataKey => $dataVal) {
            // print_r($dataKey);
            $goodsDeliveryKey = $newarr[$dataKey]['goodsDeliveryKey'];
            
            if($newarr[$dataKey]['goodsDeliveryBundle'] == 'y'){
                $goodsDeliveryKey = $newarr[$dataKey]['goodsDeliveryKey'];
            }            
            
            
            $goodsNo = $newarr[$dataKey]['goodsNo'];
            
            if($dataVal['goodsDeliveryBundle'] == 'y'){
                $mData[$goodsDeliveryKey][$dataKey] = array(
                    'goodsNo'=>$newarr[$dataKey]['goodsNo'],
                    'deliverySno'=>$newarr[$dataKey]['deliverySno'],
                    'deliveryType'=>$newarr[$dataKey]['deliveryType'],
                    'goodsDeliveryKey'=>$newarr[$dataKey]['goodsDeliveryKey'],
                    'goodsDeliveryPrice' => $newarr[$dataKey]['goodsDeliveryPrice']
                );
            }
        ...
        $_lastData = '';
        foreach($mData as $goodsDeliveryKey => $value){
            $p = 0;
            $_min = 0;
            $_max = 0;
            $deliveryType = '';
            foreach($value as $dataKey => $val){
                ...
                if($val['deliveryType'] == 'ROW'){
                    $deliveryType = 'ROW';
                    if($p == 0){
                        $_min = $val['goodsDeliveryPrice'];
                        $_lastData = $val['deliverySno'];
                    }
                    if($_min > $val['goodsDeliveryPrice']){
                        $_min = $val['goodsDeliveryPrice'];
                        $_lastData = $val['deliverySno'];
                    }
                    if($val['goodsDeliveryPrice'] == 0){
                        $_min = $val['goodsDeliveryPrice'];
                        $_lastData = $val['deliverySno'];
                        break;
                    }
                ...
                }else if($val['deliveryType'] == 'HIGH'){
                    $deliveryType = 'HIGH';
                    if($p == 0){
                        $_max = $val['goodsDeliveryPrice'];
                        $_lastData = $val['deliverySno'];
                    }
                    if($_max < $val['goodsDeliveryPrice']){
                        $_max = $val['goodsDeliveryPrice'];
                        $_lastData = $val['deliverySno'];
                    }
                    if($val['goodsDeliveryPrice'] == 0){
            ...
            $aData[$goodsDeliveryKey] = array(
                'realDeliverySno' => $_lastData,
        ...
        $price = 0;
        foreach($aData as $goodsDeliveryKey => $value){
            ...
            foreach ($newarr as $newkey => $newvalue) {
                if($newvalue['goodsDeliveryKey'] == $goodsDeliveryKey){
                    $newarr[$newkey]['lastCharge'] = $price;
                }
            }
        }
        ...
        foreach($newarr as $key => $value){
            ...
            $scmNo = (int)$value['scmNo'];
            $deliverySno = $value['deliverySno'];
            $goodsDeliveryKey = $value['goodsDeliveryKey'];
                    
            if($value['lastCharge']){
                $price = $value['lastCharge'];

                $strSQL = "SELECT sdc.* FROM es_scmDeliveryCharge sdc LEFT JOIN es_scmDeliveryBasic sdb ON sdb.sno = sdc.basicKey WHERE sdc.price = {$price} AND sdb.seeChecks = 'n' AND sdb.overloadChecks = 'n'";
                $result = $db->query_fetch($strSQL);
                
                if(!$result){
                    if(!$snoChecks[$goodsDeliveryKey][0]){
                        $basicData = [
                            'es_scmDeliveryBasic',
                            array(
                                'managerNo' => [1,'i'],
                                ...
                                'freeFl' => ['n', 's'],
                                'pricePlusStandard' => [' ','s'],
                                'priceMinusStandard' => [' ','s'],
                                'areaGroupNo' => [0,'i'],
                                'deliveryMethodFl' => ['delivery^|^^|^^|^^|^^|^','s'],
                                'regDt' => [date("Y-m-d H:i:s"),'s'],
                                'seeChecks' => ['n','s']
                            )
                        ];
                        $sno = $this->WibInsert($basicData);
                        if($sno){
                            ...
                            $this->WibInsert($chargData);
                        }
                    }else if($snoChecks[$goodsDeliveryKey][0]){
                        $sno = $snoChecks[$goodsDeliveryKey][0];
                    }
                    
                    
                }else{
                    $sno = $result[0]['basicKey'];
                }
                $overloadChecks = [
                    'es_scmDeliveryBasic',
                    array('overloadChecks'=>['y','s']),
                    array('sno'=>[$sno,'i'])
                ];
                $this->WibUpdate($overloadChecks);
                unset($overloadChecks);
                $deliverySno = $sno;
                
                $snoChecks[$goodsDeliveryKey][] = $sno;
        ...
        foreach($newarr as $key => $value){
            foreach($snoChecks as $k => $val){
                if($newarr[$key]['goodsDeliveryKey'] == $k){
                    $newarr[$key]['deliverySno'] = $val[0];
                }
            }
        }
        ...
        $overloadChecks = [
            'es_scmDeliveryBasic',
            array('overloadChecks'=>['n','s'])
        ];
        $this->WibUpdate($overloadChecks);
        unset($overloadChecks);
            
        foreach($getData as $fkey => $final){
            foreach($newarr as $nval){
                if($nval['goodsNo'] == $final['goodsNo']){
                    $getData[$fkey]['deliverySno'] = $nval['deliverySno'];
                }
            }
        }
    ...
    public function WibBundle($data)
    {
