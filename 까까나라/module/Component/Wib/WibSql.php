...
class WibSql 
{
    ...
    public function modfiy($data)
    {
        
        
        $strSQL = "SELECT * FROM es_scmDeliveryBundle WHERE sno = {$data['sno']}";
    ...
    public function getDelivery($sno)
    {
        ...
        $this->db->bind_parm_push($arrBind,'s',$sno);
    ...
    public function delete($arrData)
    {
        ...
        foreach($arrData['deliverChk'] as $dataSno){
            ...
            if($getData['cnt'] == 0){
                ...
                $this->db->set_delete_db('es_scmDeliveryBundle', 'sno = ?', $arrBind);
                $success++;
            }
        }
