...
class GoodsTogetherListController extends \Controller\Mobile\Controller
{
    public function index()
    {
        try {
            ...
            $goodsView = [];
            // 상품 정보
            foreach ($postValue['goodsNo'] as $key => $value){
                $goodsView[$key] = $goods->getGoodsView($value);
            
            
                if ($goodsView[$key]['onlyAdultFl'] == 'y' && gd_check_adult() === false && $goodsView[$key]['onlyAdultImageFl'] =='n') {
                    $goodsView[$key]['image']['detail']['thumb'][0] = SkinUtils::makeImageTag("/data/icon/goods_icon/only_adult_pc.png", '68');
                }
                ...
                if($postValue['sno']) {
                    ...
                    if($optionInfo) {
                        ...
                        if($goodsView[$key]['optionDisplayFl'] =='d' && $optionInfo['optionSno'] ) {
                            foreach($goodsView[$key]['option'] as $k => $v) {
                ...
                $goodsNm = StringUtils::stripOnlyTags($goodsView[$key]['goodsNm']);
                if (empty($goodsNm) === false) {
                    // htmlentities 상품명에 "만 들어가는 경우에 대한 처리를 위해 추가
                    $goodsView[$key]['goodsNm'] = htmlentities($goodsNm);
                }

                // 분리형 옵션인 경우, 노출안함 처리된 1차 옵션값 제거 처리
                if ($goodsView[$key]['optionDisplayFl'] === 'd') {
                    foreach ($goodsView[$key]['option'] as $k => $goodsOptionInfo) {
                        if ($goodsOptionInfo['optionViewFl'] !== 'y') {
                            unset($goodsView[$key]['option'][$k]);
                        }
                    }
                    foreach ($goodsView[$key]['option'] as $k => $goodsOptionInfo) {
                        $optionArr[$k] = $goodsOptionInfo['optionValue1'];
                    }
                    $goodsView[$key]['optionDivision'] = array_unique($optionArr);
                }

                // default 구매 최소 수량
                $goodsView[$key]['defaultGoodsCnt'] = 1;
                if($goodsView[$key]['fixedOrderCnt'] == 'option') {
                    $goodsView[$key]['defaultGoodsCnt'] = $goodsView[$key]['minOrderCnt'];
                }
                if($goodsView[$key]['fixedSales'] != 'goods' && ($goodsView[$key]['salesUnit'] > $goodsView[$key]['defaultGoodsCnt'])) {
                    $goodsView[$key]['defaultGoodsCnt'] = $goodsView[$key]['salesUnit'];
                }
            }
