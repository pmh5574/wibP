                ...
                <td colspan="3">
                    <div class="form-inline">
                        <select class="form-control" name="searchField">
                            <option value="storeTitle" <?php if ($req['searchField'] == 'storeTitle') echo 'selected' ?>>
                                매장명
                            </option>
                            <option
                                value="storeSiDo" <?php if ($req['searchField'] == 'storeSiDo') echo 'selected' ?>>지역
                            </option>
							   <option
                                value="address" <?php if ($req['searchField'] == 'address') echo 'selected' ?>>주소
                            </option>
...
<form name="frmList" id="frmList" action="article_ps.php" method="post">
    ...
    <table id="listTbl" cellpadding="0" cellspacing="0" class="table table-rows table-fixed" >
        <thead >
        <tr>
            <th class="width-3xs"><input type="checkbox" class="js-checkall" data-target-name="sno"></th>
            <th class="width-3xs">번호</th>
			<th class="width-sm">매장명</th>
            <th class="width-sm">지역</th>
            <th class="width-m">주소</th>
            <th class="width-sm">전화번호</th>
            <th class="width-sm">등록일</th>
			<th class="width-3xs">폐점여부</th>
            <th class="width-3xs">수정</th>
        </tr>
        </thead>
        <?php
        if (gd_array_is_empty($bdList['list']) === false) {
            foreach ($bdList['list'] as $val) {
                ...
                <tr class="center">
					...
					<td>
					    <?php if ($val['storeTitle']) 
                            echo gd_isset($val['storeTitle']); ?>
					</td>
					<td>
					   <?php if ($val['storeSiDo'])
                            echo gd_isset($val['storeSiDo']); ?>
					</td>
					<td class="left">
					   <?php if ($val['address'])
                            echo gd_isset(str_replace("^|^"," ",$val['address'])); ?>
					</td>
					<td>
					   <?php if ($val['storePhoneNo'])
                            echo gd_isset($val['storePhoneNo']); ?>
					</td>
					
					<td>
					   <?php if ($val['regDt'])
                            echo gd_isset($val['regDt']); ?>
					</td>
					<td>
					   <?php if ($val['storeDisplayFl'])
                            echo gd_isset($val['storeDisplayFl']); ?>
					</td>
                    <td>
...
<form name="frmList" id="frmList" action="article_ps.php" method="post">
    ...
    <table id="listTbl" cellpadding="0" cellspacing="0" class="table table-rows table-fixed">
        ...
        <tr>
            ...
            <?php if ($req['bdId'] == 'event') { ?>
                <th class="width-sm">이벤트 노출 여부</th>
            <?php } ?>
        </tr>
        </thead>
        <?php
        if (gd_array_is_empty($bdList['list']) === false) {
            foreach ($bdList['list'] as $val) {
                ...
                <tr class="center">
                    ...
                    <?php 
                        $this->db = \App::load('DB');
                        $strSQL = "SELECT * FROM es_bd_event WHERE sno = ".$val['sno'];
                        $result=$this->db->query_fetch($strSQL,null); 
                    ?>
                    <td>
                    <?php if ($req['bdId'] == 'event') {  ?>
                                <a onclick="eventDisplayFl('n', <?= $val['sno'] ?>);"
                                   class="btn btn-white btn-sm display-y<?= $val['sno'] ?>"
                                   <?php if($result[0]['eventDisplayFl'] == 'n') echo 'style=display:none;' ?>>ON</a>
                                <a onclick="eventDisplayFl2('y', <?= $val['sno'] ?>);"
                                   class="btn btn-white btn-sm display-n<?= $val['sno'] ?>"
                                   <?php if($result[0]['eventDisplayFl'] == 'y') echo 'style=display:none;' ?>>OFF</a>
                    <?php } ?>
                    </td>
                </tr>
                <?php
    ...
    <script>
        ...
        function eventDisplayFl(Fl,sno){
            $.ajax({
                url : '../board/event_display_fl.php',
                method : 'post',
                data : {'Fl' : Fl,
                        'sno' : sno 
                        },
                success : function(data){
                    alert('이벤트이 보여주기가 해제 됐습니다.');
                    $('.display-n'+sno).show();
                    $('.display-y'+sno).hide();
                }
            });
        }
        function eventDisplayFl2(Fl,sno){
            $.ajax({
                url : '../board/event_display_fl.php',
                method : 'post',
                data : {'Fl' : Fl,
