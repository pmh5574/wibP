            ...
            if (gd_isset($data)) {
                ...
                foreach ($data as $key => $val) {
                    ...
                    <tr>
                        ...
                        ?>
                        <td class="center">
                            <button class="btn btn-white btn-sm sale-on<?=number_format($page->idx); ?>" onclick="saleon(<?=number_format($page->idx); ?>,<?=$val['goodsNo']?>)"
                            <?php if($result[0]['goodsDiscountPer'] == 'y') echo 'style=display:none;' ?>
                            <?php if($result[0]['goodsPrice'] >= $result[0]['fixedPrice']) echo 'disabled' ?>>
                                OFF
                            </button>
                            <button class="btn btn-white btn-sm sale-off<?=number_format($page->idx); ?>" onclick="saleoff(<?=number_format($page->idx); ?>,<?=$val['goodsNo']?>)"
                            <?php if($result[0]['goodsDiscountPer'] == 'n' || $result[0]['goodsDiscountPer'] == null) echo 'style=display:none;' ?>
                            <?php if($result[0]['goodsPrice'] >= $result[0]['fixedPrice']) echo 'disabled' ?>>
                                ON
                            </button>
                        </td>
                        ...
                        <td class="center">
                            <span>
                                <label class="radio-inline">
                                    <input type="radio" name="naverFl<?=$val['goodsNo']?>" value="y" onclick="naveron(<?=$val['goodsNo']?>)" <?php if($result[0]['naverFl'] == 'y') echo 'checked'; ?>>노출함
                                </label>
                            </span>
                            <span>
                                <label class="radio-inline">
                                    <input type="radio" name="naverFl<?=$val['goodsNo']?>" value="n" onclick="naveroff(<?=$val['goodsNo']?>)" <?php if($result[0]['naverFl'] == 'n') echo 'checked'; ?>>노출안함
                                </label>
                            </span>
                        </td>
                    </tr>
...
<script type="text/javascript">
    ...
    function saleon(pcnt,goodsNo){
        ...
        $.ajax({
            url : '../goods/goods_discount_per.php',
            method : 'post',
            data : {'saleon' : 'on',
                    'goodsNo' : goodsNo 
                    },
    ...
    function naveron(goodsNo){
        ...
        $.ajax({
            ...
            success : function(data){
                alert('네이버쇼핑 노출여부가 설정 됐습니다.');
            }
        });

    }

    function naveroff(goodsNo){
