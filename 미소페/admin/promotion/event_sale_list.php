        ...
        if (gd_isset($data)) {
            foreach ($data as $key => $val) {
                ...
                <tr>
                    ...
                    <?php 
                        $db = \App::load('DB');
                        $strSQL = "SELECT * FROM es_displayTheme WHERE sno = ".$val['sno'];
                        $result=$db->fetch($strSQL);
                    ?>
                    <td class="center">
                        <button class="btn btn-white btn-sm wib-btn want-on<?=number_format($page->idx); ?>" onclick="wanton(<?=number_format($page->idx); ?>,<?=$val['sno']?>)"
                        <?php if($result['wantChecks'] == 'y') echo 'style=display:none;' ?>
                        <?php if($val['statusText'] == '종료') echo 'disabled' ?>>
                            OFF
                        </button>
                        <button class="btn btn-white btn-sm wib-btn want-off<?=number_format($page->idx); ?>" onclick="wantoff(<?=number_format($page->idx); ?>,<?=$val['sno']?>)"
                        <?php if($result['wantChecks'] == 'n') echo 'style=display:none;' ?>
                        <?php if($val['statusText'] == '종료') echo 'disabled' ?>>
                            ON
                        </button>
                    </td>
