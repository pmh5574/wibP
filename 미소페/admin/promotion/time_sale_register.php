<script type="text/javascript">
    ...
    function setAddGoods(frmData) {
        ...
        $.each(frmData.info, function (key, val) {
            ...
            //추가
            if(!val.eachBenefit) val.eachBenefit = '';
            ...
            addHtml += '<input type="hidden" name="itemEachBenefit[]" value="'+val.eachBenefit+'" />';
            ...
            addHtml += '<td class="center js-goodschoice-hide"><div class="form-inline" style="position: relative;">각 상품의 판매가 - <input type="text" name="eachBenefit[]" class="form-control width-2xs js-number-only" value="'+val.eachBenefit+'" maxlength="3"> % (판매가기준)</div></td>';
            addHtml += '</tr>';
                    ...
                    foreach ($data['goodsNo'] as $key => $val) {
                        ...
                        <tr id="tbl_add_goods_<?=$val['goodsNo']; ?>" <?php if ($data['fixGoodsNo'] && in_array($val['goodsNo'], array_values($data['fixGoodsNo']))) { ?>style='background:#d3d3d3' class="add_goods_fix" <?php } else { ?>class="add_goods_free" <?php } ?>>
                            <td class="center">
                                ...
                                <input type="hidden" name="itemEachBenefit[]" value="<?= gd_isset($val['eachBenefit']) ?>" />
                            ...
                            <td class="center js-goodschoice-hide">
                              <div class="form-inline" style="position: relative;">
                                  각 상품의 판매가 - 
                                  <input type="text" name="eachBenefit[]" class="form-control width-2xs js-number-only" value="<?php if($val['eachBenefit']){echo $val['eachBenefit'];} ?>" maxlength="3"> 
                                  % (판매가기준)
                              </div>
                            </td>
                        </tr>
