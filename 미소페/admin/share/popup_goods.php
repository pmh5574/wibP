...
<table cellpadding="0" cellspacing="0" width="100%" border="0" class="goodsChoice_outlineTable" style="margin-left:-10px;">
                        ...
                        <table class="table table-cols" style="border-top: 0px">
                            ...
                            <tr>
                                ...
                                <td colspan="3"><div class="form-inline">
                                        ...
                                        <textarea name="keyword" class="form-control" placeholder="복수 검색&#13;&#10;(enter로 구분)"><?=$search['keyword']; ?></textarea>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                            ...
                            foreach ($data as $key => $val) {
                                ...
                                <tr id="tbl_add_goods_<?php echo $val['goodsNo'];?>" class="add_goods_free">
                                    <td class="center">
                                        ...
                                        <input type="hidden" name="itemEachBenefit[]" value="" />
                                    </td>
            ...
            <table cellpadding="0" cellpadding="0" width="100%">
                ...
                <td valign="top" class="goodsChoice_registeredTdArea" >
                    ...
                    <div id="goodsChoice_registerdOutlineDiv">
                        ...
                        <tbody id="mSortable" contents-length="<?=is_null($setGoodsList) ? 0 : strlen(trim($setGoodsList))?>">
                                ...
                                <td>
                                    <table cellpadding="0" cellspacing="0" width="100%">
                                        <tr>
                                            ...
                                            <td>
                                                <input type="button" value="일괄 이동" class="btn btn-white total_goodsChoice_moveBtn">
                                            </td>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            /* 추가 */
            $('.goodsChoiceConfirm').unbind("click").bind("click",function (e) {
...
//            $( "#mSortable" ).sortable({
//               cancel: ".add_goods_fix"
//            }).disableSelection();
            //추가끝
            
//            $('.number').on('click',function(){
//                var oriNum = $(this).html();
//                $(this).html('<input type="text" value="'+oriNum+'">');
//                console.log($(this).html());
//            });
// 
            

            $('.total_goodsChoice_moveBtn').click(function(){
                var _row = $("#mSortable").find('tr');
                var _rowLength = _row.length;
                var totalRowCount = _rowLength - 1;
                var lastFl = false;
                var _array = [];
                var _checks = false;
                var _checksNum = 1;
                ...
                redChecks(_row,_rowLength);
                
                var _list = [];
                var _checksList = [];
                
                $(_row).each(function(index){
                    
                    var _move = $(this).find('.misoSort').val();
                    var _wibChecks = $(this).find('.wibSort').val();
                    var _this = $(this);
                    var _rnmove= $(this).prevAll().length;
                    
                    
                    if(_move){
                        _checksList[_move] = $(this);
                    }else if(!(_move)){
                        _list[_checksNum] = $(this);
                        _checksNum++;
                    }
                    
                    
                    
                    if($(this).find('.misoSort').hasClass('rn')){
                        _array[_rnmove] = $(this);
                        _checks = true;
                    }else if(!($(this).find('.misoSort').hasClass('rn'))){
               ...
               _checksNum = 1;
                ...
                $(_row).each(function(index){
                    ...
                    if(!(_checksList[(_index)])){
                        _lastArray[_index] = _list[_checksNum];
                        _checksNum++;
                    }else if(_checksList[(_index)]){
                        _lastArray[_index] = _checksList[_index];
                    }
                });


                if(_checks){
                    alert('중복된 진열순서가 있습니다.');
                }else{

                    $('#mSortable tr').remove();
                    
                    $('#mSortable').append(_lastArray);
        ...
        function wibEach(){
            $('#mSortable .number').each(function(index){
               
               var _split= $(this).attr('class').split('addGoodsNumber_');
               
               var oriNum = $(this).html();
               $(this).html('<input type="hidden" class="wibSort" value="'+(index+1)+'">');
               $(this).html('<input type="hidden" class="wibSort" value="'+(index+1)+'"> <input type="text" class="misoSort wib_'+_split[1]+'">');
           });
        }
        
        

        function redChecks(row,rowLength){
            $(row).each(function(index){
                var _move = $(this).find('.misoSort').val();
                var _checks = false;
                
               if(_move){
                   if($(this).find('.misoSort').hasClass('rn') == false){
                    
                    for(var i=0;i<rowLength;i++){
                         if(i != index){
                             var _moreChecks = $("#mSortable tr").eq(i).find('.misoSort').val();

                             if(_move == _moreChecks){

                                 $(this).find('.misoSort').addClass('rn');
                    ...
                    if(_checks == false){
                        $(this).find('.misoSort').removeClass('rn');
                    }
                }
               }
        ...
        $(document).on('change','#mSortable .misoSort', function(e){
            $(this).parents('tr').addClass('an');
        });
...
//        $(document).on('blur','.misoSort',function(e){
//            
//            //백스페이스바 제외
//            if(e.keyCode == 8){
//                return false;
...
//            var _goodsNo = _this.split('wib_');
//            var _nowVal = $(this).val();
//            var selfChecks = false;
//
//            // $('#'+registeredTableID+' input[name="itemGoodsNo[]"]').not(':checked').each(function (num)
//            // aa = aa.substr(0, aa.length -1); 
//            
//            if(_nowVal > _rowLength){
//                _nowVal = _nowVal.substr(0, _nowVal.length-1);
//                $(this).parents('tr').addClass('rn');
//                $(this).focus();
//                
//                
//                return false;
//            }else{
//                if($(this).parents('tr').hasClass('rn')){
//                    console.log('??');
//                    $(this).parents('tr').removeClass('rn');
//                }
//            }
//
//            $('#mSortable .misoSort').not($(this)).each(function(){
//                var _checksVal = $(this).val();
//                if(_checksVal == _nowVal){
////                    selfChecks = true;
////                    console.log(_goodsNo[1]);
////                    $('#tbl_add_goods_'+_goodsNo[1]).addClass('rn');
//                    $(this).parents('tr').addClass('rn');
//                    $(this).focus();
//                    return false;
//                }else if(_checksVal != _nowVal){
//                    
//                    if($(this).parents('tr').hasClass('rn')){
//                        console.log('aa?!');
////                        $('#tbl_add_goods_'+_goodsNo[1]).removeClass('rn');
//                        $(this).parents('tr').removeClass('rn');
//                    }
//                    
//                    
