        ...
        while ($data = $this->db->fetch($result)) {
            ...
            if (gd_is_plus_shop(PLUSSHOP_CODE_TIMESALE) === true && Request::post()->get('mode') !== 'cartEstimate') {
                ...
                if ($timeSaleInfo) {
                    ...
                    //추가 ||기준으로 자른후 goodsNo번호에 맞는 번호를 대입
                    ...
                    if ($data['goodsPrice'] > 0) {
                        ...
                        if($timeSaleInfo['timeSaleEachBenefit']){
                            $data['timeSalePrice'] = gd_number_figure((($timeSaleInfo['timeSaleEachBenefit'] / 100) * $data['goodsPrice']), $truncGoods['unitPrecision'], $truncGoods['unitRound']);

                            $data['goodsPrice'] = gd_number_figure($data['goodsPrice'] - (($timeSaleInfo['timeSaleEachBenefit'] / 100) * $data['goodsPrice']), $truncGoods['unitPrecision'], $truncGoods['unitRound']);
                        }else{
                    ...
                    if($data['optionFl'] === 'y'){
                        // 타임세일 할인금액
                        if($timeSaleInfo['timeSaleEachBenefit']){
                            $data['timeSalePrice'] = gd_number_figure($data['timeSalePrice'] + (($timeSaleInfo['timeSaleEachBenefit'] / 100) * $data['optionPrice']), $truncGoods['unitPrecision'], $truncGoods['unitRound']);

                            $data['optionPrice'] = gd_number_figure($data['optionPrice'] - (($timeSaleInfo['timeSaleEachBenefit'] / 100) * $data['optionPrice']), $truncGoods['unitPrecision'], $truncGoods['unitRound']);
                        }else{
