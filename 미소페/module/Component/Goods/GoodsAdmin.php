...
{
    public function setBatchTotal($getData)
    {
        $arrGoodsNo = [];
        $goodsPrice = [];
        ...
        foreach ($getData['arrGoodsNo'] as $k => $v) {
            ...
            $goodsPrice[$tmp[0]] = $getData['goodsPrice'][$tmp[0]];
        ...
        $this->setBatchUpdate($goodsData, array('goodsPrice'));

        $applyFl = parent::setBatchStock($getData);
        return $applyFl;
        ...
        if ($this->search['key'] && $this->search['keyword']) {
            if ($this->search['key'] == 'all') {
                ...
                if ($this->search['key'] == 'companyNm') {
                    ...
                    //추가
                    $_search = $this->search['keyword'];
                    $_enter  = nl2br($_search,'<br />');

                    if(strpos($_enter) !== false){

                        $addSearch = '';
                        $addWhere  = '';

                        //엔터값이 있는얘들 엔터값마다 배열로 만들기
                        $addSearch = explode("\n", $_search);
    ...
    public function saveInfoGoods($arrData)
    {
        ...
        $sqlArr = array(
            DB_GOODS, 
            array('season' => array(implode(",", $arrData['season']), 's')),
            array( 'goodsNo' => array($this->goodsNo, 'i') )
        );
        // 위브 클래스 연결
        $conn = new WibSql();
        // 데이터 업데이트
        $conn->WibUpdate($sqlArr);
    ...
    public function getAdminListGoods($mode = null, $pageNum = 5)
    {
        ...
        //추가
        if($getValue['sort'] == 'g.goodsNo desc' && $getValue['keyword']){
        
            $_search = $getValue['keyword'];
            $_enter  = nl2br($_search,'<br />');
            ...
            if(strpos($_enter) !== false){
                ...
                foreach($addSearch as $key => $value){
                    ...
                    if(trim($value) == false){
                        ...
                        $_sort .= ' WHEN g.'.$getValue['key'].' LIKE "%'.$value.'%" THEN '.$key.' ';
                        
                    }
                }
                
                $_sort .= ' END) ASC, '.$getValue['sort'];
                
                Request::get()->set('sort', $_sort);
            }
        
        }

        $getData = parent::getAdminListGoods($mode, $pageNum);
        return $getData;
    }
    ...
    public function getAdminListBatch($mode = null)
    {
        ...
        $getData = parent::getAdminListBatch($mode);
        return $getData;
    }
    ...
    public function getAdminListOptionBatch($mode = null)
    {
        ...
        $getData = parent::getAdminListOptionBatch($mode);
        return $getData;
    }
}
