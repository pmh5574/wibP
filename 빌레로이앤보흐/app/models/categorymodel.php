...
class categorymodel extends CI_Model {
	...
	public function get_list($code,$arrWhere=array(),$division=null) {
		...
		/*######################## 19.04.29 gcs jdh : #2092 개선 s */
		if($division=='searchForm' && in_array(uri_string(),array('mshop','mshop/index', 'mshop/category'))){

			//######################## 19.05.15 gcs yun jy : m 이 없어지고 code값이 있을 때 처리
			if($_GET['m']) {
				$targetProviderSeq = $_GET['m'];
			}else{
				$targetProviderSeq = $_GET['code'];		
			}

			$provider_seq = preg_replace("/[^0-9]/","",$targetProviderSeq);
			...
			if($provider_seq){
				...
				LEFT JOIN 
					fm_category_link cl 
						ON g.goods_seq = cl.goods_seq and
						length(cl.category_code) = '".$len."' 
				WHERE 
					g.provider_seq='".$provider_seq."'
				GROUP BY 
					cl.category_code"
				);
				foreach ($query2->result_array() as $crow){
					$in_category[] = $crow['category_code'];
				}
				$arrWhere[] = "a.category_code in ('".implode("','",$in_category)."')";
			}
		}
	...
	public function get_list_mshop($code,$arrWhere=array(),$division=null) {

		if($division=='searchForm' && in_array(uri_string(),array('mshop','mshop/index', 'mshop/category'))){
			
			$provider_seq = preg_replace("/[^0-9]/","",$_GET['m']);
			$sql = "SELECT b.`category_code` FROM `fm_category_link` b 
									WHERE b.goods_seq in (SELECT goods_seq FROM fm_goods c WHERE c.provider_seq='{$provider_seq}') 
									GROUP BY b.category_code";
			$query = $this->db->query($sql);
			$categorys = array();
			foreach ($query->result_array() as $row){
				$categorys[] = $row['category_code'];
			} //foreach ($query->result_array() as $row)
			
			$category		= implode('", "', $categorys);
			$where[] = "a.category_code in (\"{$category}\")";
	...
	public function get_category_recommend_display_seq($categoryCode){
		...
		if($categoryData['recommend_display_seq'] && $displayData){
			...
			return array('recommend_display_seq'=>$categoryData['recommend_display_seq'],'m_recommend_display_seq'=>$m_recommend_display_seq);
		}else{
			$data = array(
				'kind' => 'category',
				'count_h' => '1',
			...
			$query = "update fm_category set recommend_display_seq=?,m_recommend_display_seq=? where category_code=?";
			$this->db->query($query,array($recommend_display_seq,$m_recommend_display_seq,$categoryCode));

			return array('recommend_display_seq'=>$recommend_display_seq,'m_recommend_display_seq'=>$m_recommend_display_seq);
		}
	}
	...
	public function reSortAll($categoryCode){
		//#458 기존 sort 무시하고 상품등록순으로 전체-재정렬 됩니다. gcs ysm @2018-12-18
		$query = $this->db->query("select l.category_link_seq from fm_goods_option o, fm_goods g, fm_category_link l where g.goods_view='look' AND g.goods_type = 'goods' and o.goods_seq=g.goods_seq and l.goods_seq = g.goods_seq and o.default_option ='y' and l.category_code = ? group by g.goods_seq order by g.regist_date desc", $categoryCode);//l.sort asc, 
		$sort	= 0;
	...
	public function getSortValue($categoryCode, $type){
		switch($type){
			############################## 2019-06-20 [gcs] sdh : 카테고리 정렬 쿼리 수정(속도문제) ##############################
			/*
			case 'min':
			...
			break;
			case 'mobile_min':
				$query		= $this->db->query("select min(l.mobile_sort) sortVal from fm_goods_option o, fm_goods g, fm_category_link l where g.goods_view='look' AND g.goods_type = 'goods' and o.goods_seq=g.goods_seq and l.goods_seq = g.goods_seq and o.default_option ='y' and l.category_code = ? ", $categoryCode);
				$sortData	= $query->row_array();
			break;
			case 'mobile_max':
				$query		= $this->db->query("select max(l.mobile_sort) sortVal from fm_goods_option o, fm_goods g, fm_category_link l where g.goods_view='look' AND g.goods_type = 'goods' and o.goods_seq=g.goods_seq and l.goods_seq = g.goods_seq and o.default_option ='y' and l.category_code = ? ", $categoryCode);
				$sortData	= $query->row_array();
			break;
			*/
			case 'min':	$funcSql = "min(l.sort)"; $distinctSql = "sort"; break;
			case 'max':	$funcSql = "max(l.sort)"; $distinctSql = "sort"; break;
			case 'cnt':	$funcSql = "count(l.category_link_seq)"; $distinctSql = "category_link_seq"; break;
			case 'mobile_min':	$funcSql = "max(l.mobile_sort)"; $distinctSql = "mobile_sort"; break;
			case 'mobile_max':	$funcSql = "max(l.mobile_sort)"; $distinctSql = "mobile_sort"; break;
		}
		$sql		= "select $funcSql sortVal FROM (SELECT goods_seq FROM fm_goods WHERE goods_type = 'goods' AND goods_view='look') g INNER JOIN  (SELECT DISTINCT $distinctSql,goods_seq FROM fm_category_link WHERE category_code = ?) l ON l.goods_seq = g.goods_seq";
		$query		= $this->db->query($sql, $categoryCode);
		$sortData	= $query->row_array();
		############################## 2019-06-20 [gcs] sdh : 카테고리 정렬 쿼리 튜닝(속도문제) ##############################
	...
	public function getChildCategory($code,$exactly=false,$division='catalog'){
		...
		/*  //@nsg 카테고리 네비게이션에서 제외인 경우, 미니샵에서 미노출되는 오류로 주석처리 2018-06-22
		switch(uri_string()){
			case "mshop":
			case "mshop/index":
				$arrWhere[] = "hide != '1'";
