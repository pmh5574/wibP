...
<form name="frmWrite" id="frmWrite" action="article_ps.php" method="post" enctype="multipart/form-data">
    ...
    <table class="table table-cols" id="board-table">
		...
		<tr>
			<th class="require">영상링크</th>
			<td>
				<input type="text" name="urlLink" id="urlLink" value="<?= gd_isset($bdWrite['data']['urlLink']) ?>" style="width:500px;padding: 4px 12px;border: 1px solid #D5D5D5;">	
					<script>
						$("#urlLink").blur(function(){
						   let input = $('#urlLink');
						   let content = input.val();
						   content = content.replace('https://www.youtube.com/watch?v=', 'https://www.youtube.com/embed/'); // 把 World 替换为 Hello
						   input.val(content);
						});
					</script>
			</td>
        ...
        <tr>
            ...
            <td>
                <div>
                    <?php
                    if (gd_isset($bdWrite['data']['groupThread']) == '' && $req['mode'] != 'reply' && gd_isset($bdWrite['cfg']['bdKind']) != 'event') {
                        ?>
