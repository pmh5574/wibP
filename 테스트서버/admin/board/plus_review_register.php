<form name="frmWrite" id="frmWrite" action="plus_review_ps.php" method="post" enctype="multipart/form-data">
    ...
    <table class="table table-cols">
        ...
        <?php 
            if($req['bestadd']=="bestadd"){ 
                    $fileNm = $req['bestUploadFileNm'];
                    $bestUploadFileNm = explode("^|^",$fileNm); 
                    $req['bestUploadFileNm'] = $bestUploadFileNm;
                ?>
            <tr>
                <th>베스트리뷰 Before/After</th>
                <td>
                    <ul class="pdl0" id="uploadBox">
                        <?php
                        if (count($data['uploadedFile']) <= $config['uploadMaxCount']) { ?>
                            <li class="form-inline mgb5">
                                <input type="file" name="beforeimages">
                                <span>Before</span>
                                <span></span>
                            </li>
                            <?php 
                            if($req['bestUploadFileNm'][0] != ''){ ?>
                                <li class="form-inline mgb5">    
                                    <span>Before에 등록된 사진 이름: <?=$req['bestUploadFileNm'][0]?></span>
                                </li>
                            <?php } ?>
                                <li class="form-inline mgb5">
                                    <input type="file" name="afterimages">
                                    <span>After</span>
                                </li>
                            <?php 
                            if($req['bestUploadFileNm'][0] != ''){ ?>
                                <li class="form-inline mgb5">    
                                    <span>After에 등록된 사진 이름: <?=$req['bestUploadFileNm'][1]?></span>
                                </li>
                            <?php } ?>
                        <?php } ?>
                    </ul>
                    <div class="notice-info">
                        Before와 After사진을 등록해주세요.<br>등록안하고 저장시 삭제됩니다.
                    </div>
                </td>
            </tr>
        <?php
