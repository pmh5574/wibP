<script type="text/javascript" xmlns="http://www.w3.org/1999/html">
    ...
    function fill_option_layer() {
        ...
        for ($i = 0; $i < $data['optionCnt']; $i++) {
            ...
            // 2019-10-02 추가 : 박사무엘
            echo "  $(':radio[name=\"optionY[optionGubun][" . $i . "]\"][value=\"" . $data['wbOptionGubun'][$i] . "\"]').prop('checked', 'checked');" . chr(10);
            ...
            if (is_array($data['option']['optVal'][$i + 1])) {
                ...
                // 2019-09-30 추가 : 박사무엘
                // 옵션값 세팅하기 전에 먼저 호출되어야 컬러코드값을 입력할 수 있다.
                echo "setOptionValue(".$i.");" . chr(10);

                foreach ($data['option']['optVal'][$i + 1] as $key => $val) {
                    
                    // 2019-10-17 추가 : 박사무엘
                    // 옵션값에 컬러코드값이 같이 있는 경우 분리하여 각각의 폼에 대입한다.
                    //echo "	$('#option_optionValue_" . $i . "_" . $j . "').val('" . gd_htmlspecialchars_slashes($val, 'add') . "');" . chr(10);
                    $optVal = gd_htmlspecialchars_slashes($val, 'add');
                    if(strpos($optVal, '#') !== false) {
                        $arrOptVal = explode(INT_DIVISION, $optVal);
                        echo "	$('#option_optionValue_" . $i . "_" . $j . "').val('" . $arrOptVal[0] . "');" . chr(10);
                        echo "	$('input[data-text-id=\'option_optionValue_" . $i . "_" . $j . "\']').val('" . $arrOptVal[1] . "');" . chr(10);
                    } else {
                        echo "	$('#option_optionValue_" . $i . "_" . $j . "').val('" . gd_htmlspecialchars_slashes($val, 'add') . "');" . chr(10);
                    }
    ...
    function option_setting_layer(thisCnt) {
        ...
        if (fieldCnt == '0' && fieldChk > 0) {
            ...
            templateHtml += '<tr>';
            templateHtml += '<th class="left" style="width:200px">구분</th>';
        ...
        if (fieldChk > 0) {
            ...
            for (var i = fieldCnt; i < thisCnt; i++) {
                ...
                var optionDisplay = $(":radio[name='optionY[optionDisplayFl]']:checked").val();
                if(optionDisplay == 'c') {
                    addHtml += '<td><input type="radio" name="optionY[optionGubun]['+i+']" value="normal" onclick="setOptionValue('+i+')" checked><label> 일반 </label>';
                    addHtml += ' <input type="radio" name="optionY[optionGubun]['+i+']" value="color" onclick="setOptionValue('+i+')"><label class="colorchip-label"> 색상 </label>';
                    addHtml += ' <input type="radio" name="optionY[optionGubun]['+i+']" value="size" onclick="setOptionValue('+i+')"><label class="colorchip-label"> 사이즈 </label></td>';
                } else {
                    addHtml += '<td><input type="radio" name="optionY[optionGubun]['+i+']" value="normal" onclick="setOptionValue('+i+')" checked><label> 일반 </label>';
                    addHtml += ' <input type="radio" name="optionY[optionGubun]['+i+']" value="color" onclick="setOptionValue('+i+')" style="visibility: hidden" disabled><label class="colorchip-label" style="visibility: hidden"> 색상 </label>';
                    addHtml += ' <input type="radio" name="optionY[optionGubun]['+i+']" value="size" onclick="setOptionValue('+i+')" style="visibility: hidden" disabled><label class="colorchip-label" style="visibility: hidden"> 사이즈 </label></td>';
                }
                ...
                addHtml += '<input type="text" id="option_optionValue_' + i + '_0" data-option-sno="'+i+'" name="optionY[optionValue][' + i + '][]" value="" class="form-control"  style="width:330px;"  placeholder="Enter키를 이용 옵션값을 연속적으로 입력하세요. ex)XL" onblur=" if(option_value_check_layer(\'' + i + '\',\'0\') == true) { option_grid_layer(); } " maxlength="100" />';
    ...
    function option_value_conf_layer(loc, thisCnt, loadChk) {
        ...
        // 2019-10-02 추가 : 박사무엘
        // 컬러선택형으로 변경할 옵션값의 id를 배열에 저장하기 위해 배열선언
        var arrOptionValueId = [];
        ...
        if (fieldChk > 0) {
            ...
            for (var i = fieldCnt; i < thisCnt; i++) {
                ...
                addHtml += '<input type="text" id="option_optionValue_' + loc + '_' + i + '" data-option-sno="'+loc+'" name="optionY[optionValue][' + loc + '][]" value="" class="form-control" style="width:330px;" placeholder="Enter키를 이용 옵션값을 연속적으로 입력하세요. ex)XL" onblur=" if(option_value_check_layer(\'' + loc + '\',\'' + i + '\') == true) { option_grid_layer(); } " />';
                
                // 2019-10-02 추가 : 박사무엘
                // 컬러선택형으로 변경할 옵션값의 id를 배열에 저장
                // 컬러값을 임시 저장할 hidden 필드
                if( $(":radio[name='optionY[optionGubun][" + loc + "]']:checked").val() == 'color' ) {
                    arrOptionValueId.push('option_optionValue_' + loc + '_' + i);
                    addHtml += '<input type="hidden" name="optionY[optionColorValue][' + loc + '][]" data-text-id="option_optionValue_' + loc + '_' + i + '" value="">';
                }
                ...
                if (loc == 0) {
                    ...
                    addHtml += '<input type="file" name="optionYIcon[goodsImage][' + loc + '][]" value="" class="form-control width-2xl" style="height:30px" />';
        ...
        // 2019-10-02 추가 : 박사무엘
        // 옵션노출방식이 컬러/사이즈형인 경우 옵션값 추가후 컬러선택형으로 변형
        for(var idx=0; idx<arrOptionValueId.length; idx++) {
            $('#' + arrOptionValueId[idx]).removeClass('form-control').addClass('wcolpick_textform');
            $('#' + arrOptionValueId[idx]).attr('placeHolder', '색상명 입력');
            
            $('#' + arrOptionValueId[idx]).after($("<div class='wcolpick_box' data-text-id='"+ arrOptionValueId[idx] +"'></div>"));
            setWcolpick(arrOptionValueId[idx]);
        }
            ...
            if (j == 0) {
                ...
                for (var k = 0; k < optionCnt; k++) {
                    ...
                    if($('#option_optionValue_' + k + '_' + valIdNo[k]).length) {
                        ...
                        // 2019-10-17 추가 : 박사무엘
                        // 컬러색상명과 컬러코드값을 같이 저장 (나중에 분리해서 각각 옵션값과 색상표기용으로 사용)
                        if($(":radio[name='optionY[optionGubun][" + k + "]']:checked").val() == 'color') {
                            var colorVal = $('input[data-text-id="option_optionValue_' + k + '_' + valIdNo[k] + '"]').val();
                            if(colorVal !== undefined && colorVal.indexOf('#') != -1) {
                                optVal = optVal + '<?=INT_DIVISION?>' + colorVal;
                            }
                        }
                        ...
                        // 2019-09-30 추가 : 박사무엘
                        // 옵션값에 컬러코드값이 있으면 분리해서 컬러색상명 + 컬러색상 표시
                        if(optVal.indexOf('#') != -1) {
                            var arrOptVal = optVal.split('<?=INT_DIVISION?>');
                            optVal = '<div style="vertical-align:middle; display: inline-block; width: 25px; height:25px; margin: 0 auto; background:' + arrOptVal[1] + '"></div> ' + arrOptVal[0];
                        }
    ...
    function goods_image_layer(fieldID, addBtnYN, urlType) {
        ...
        if (urlType == 'y') {
            ...
            if(fieldID == 'imageFbGoods'){ // 페이스북 피드 이미지 입력하는 경우
                addHtml += '<input type="file" name="imageFb[' + fieldID + '][]" class="form-control width80p" ' + clickCheck + ' />' + imageInfo;
            }else {
    ...
    // 2019-09-30 추가 : 박사무엘
    // 구분값에 따라 옵션값 필드 셋팅
    function setOptionValue(i) {
        var gubun = $(":radio[name='optionY[optionGubun][" + i + "]']:checked").val();
        if(gubun == 'color') {
            $(":text[name='optionY[optionValue][" + i + "][]']").removeClass('form-control').addClass('wcolpick_textform');
            $(":text[name='optionY[optionValue][" + i + "][]']").attr('placeHolder', '색상선택');

            $(":text[name='optionY[optionValue][" + i + "][]']").each(function(index, item) {

                if($(this).siblings("div[data-text-id='"+ $(this).attr('id') +"']").length == 0) {

                    $(this).after($("<input type='hidden' name='optionY[optionColorValue][" + i + "][]' data-text-id='" + $(this).attr('id') + "'>"));
                    $(this).after($("<div class='wcolpick_box' data-text-id='"+ $(this).attr('id') +"'></div>"));
                    $(this).siblings("div[data-text-id='"+ $(this).attr('id') +"']").css('background-color', $(this).val());
                    setWcolpick($(this).attr('id'));
                } else {
                    $(this).siblings("div[data-text-id='"+ $(this).attr('id') +"']").css('background-color', $(this).val());
                }

                $(this).siblings("div[data-text-id='"+ $(this).attr('id') +"']").css('display', 'inline-block');
            });

        } else {
            $(":text[name='optionY[optionValue][" + i + "][]']").removeClass('wcolpick_textform').addClass('form-control');
            $(":text[name='optionY[optionValue][" + i + "][]']").attr('placeHolder', 'Enter키를 이용 옵션값을 연속적으로 입력하세요. ex)XL');
            $(":text[name='optionY[optionValue][" + i + "][]']").each(function(index, item) {
                $(this).siblings("div[data-text-id='"+ $(this).attr('id') +"']").css('display', 'none');
            });
        }
    }
    ...
    // 2019-09-30 추가 : 박사무엘
    // colorpicker를 셋팅한다.
    function setWcolpick(id) {
        var selector = "div[data-text-id='" + id + "']";
        var setColor = $("input[data-text-id='" + id + "']").val();
        setColor = (setColor !== undefined) ? setColor : '#F00';
        ...
        $(selector).loads({
            ...
            onSubmit: function(ev) {
                $('input[data-text-id=\'' + id + '\']').val('#' + ev.hex);
                $(ev.el).css('background', '#' + ev.hex);
                $(ev.el).hides();
            },
            onHide: function(ev) {
    ...
    // 2019-10-02 추가 : 박사무엘
    // 옵션노출방식 -> 컬러/사이즈형을 선택하면 구분에서 색상선택형, 사이즈형을 선택할 수 있게 옵션값이 추가된다
    // 옵션노출방식 -> 일체형(조합), 분리형(조합)을 선택하면 구분에서 색상선택형, 사이즈형을 선택할 수 없게 숨겨진다.
    function setOptionDisplay(val) {
        
        var optionCnt = $("#optionY_optionCnt").val();
        // 옵션노출방식 : 컬러/사이즈형 을 선택했을때
        if(val == 'c') {
            for(var i=0; i<optionCnt; i++) {
                $(":radio[name='optionY[optionGubun][" + i + "]'][value!='normal']").removeProp('disabled');
                $(":radio[name='optionY[optionGubun][" + i + "]'][value!='normal']").css('visibility', 'visible');
                $(".colorchip-label").css('visibility', 'visible');
            }
        // 옵션노출방식 : 일체형(조함) 또는 분리형(조합)을 선택했을때
        } else {
            for(var i=0; i<optionCnt; i++) {
                $(":radio[name='optionY[optionGubun][" + i + "]'][value='normal']").prop('checked', 'checked');
                $(":radio[name='optionY[optionGubun][" + i + "]'][value='normal']").click(); // 옵션값 컬러선택형에서 일반으로 복구
                $(":radio[name='optionY[optionGubun][" + i + "]'][value!='normal']").prop('disabled', 'disabled');
                $(":radio[name='optionY[optionGubun][" + i + "]'][value!='normal']").css('visibility', 'hidden');
                $(".colorchip-label").css('visibility', 'hidden');
            }
        }
    }

    $(document).ready(function() {
        $("div[data-text-id!='']").each(function(idx, item) {
            var id = $(item).data('text-id');
            var colorVal = $("input[data-text-id='" + id + "']").val();
            $(item).setColor(colorVal);
            $(item).css('background-color', colorVal);
        });       
    });
    //-->
</script>
    ...
    <div id="depth-toggle-layer-stockOption-popup">
        ...
        <table class="table table-cols">
            ...
            <tr>
                ...
                <td>
                    <label class="radio-inline">
                        <input type="radio" name="optionY[optionDisplayFl]" value="s" <?=gd_isset($checked['optionDisplayFl']['s']); ?> onclick="setOptionDisplay(this.value)"/>일체형(조합)
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="optionY[optionDisplayFl]" value="d" <?=gd_isset($checked['optionDisplayFl']['d']); ?> onclick="setOptionDisplay(this.value)"/>분리형(조합)
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="optionY[optionDisplayFl]" value="c" <?=gd_isset($checked['optionDisplayFl']['c']); ?> onclick="setOptionDisplay(this.value)"/>컬러/사이즈형(조합)
                    </label>
                </td>
            </tr>
            <tr>
        ...
        <div class="notice-info" style="margin-bottom:20px;">
            ...
            옵션명은 30자, 옵션값은 100자까지 등록 가능합니다. (한글, 영문 대/소문자, 숫자, 특수문자 등록 가능합니다.)<br/>
                        ...
                        <tr id="tbl_option_info_<?=$key+1?>">
                            ...
                            for ($i = 0; $i < $data['optionCnt']; $i++) {
                                ...
                                <td class="center <?=$optClass?>">
                                    ...
                                    <?php
                                        $optionValueTmp = gd_isset($data['option'][$key]['optionValue' . ($i + 1)]);
                                        if(strpos($optionValueTmp, '#') !== false) {
                                            $optionValueTmp = explode(INT_DIVISION, $optionValueTmp);
                                            echo '<div class="wcolpick_size25" style="background:' . $optionValueTmp[1] . '"></div> ';
                                            echo $optionValueTmp[0];
                                        } else {
                                            echo $optionValueTmp;
                                        }
