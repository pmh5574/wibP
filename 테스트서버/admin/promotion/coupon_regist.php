    ...
    <table class="table table-cols">
        ...
        <tr>
            ...
            <td colspan="3">
                ...
                <label class="radio-inline">
                    <input type="radio" name="couponUseType" value="code" <?= $checked['couponUseType']['code'] ?> />코드입력쿠폰
                </label>
            </td>
        </tr>
        <tr>
        ...
        <tr class="codeType" style="display:none">
            <th>생성매수</th>
            <td colspan="3">
                <div class="form-inline mgb5" style="position: relative;">
                    <input type="text" name="codeNumber" value="1" class="form-control js-number-only width-2xs" maxlength="3">매
                </div>
            </td>
        </tr>
        <tr>
...
<script type="text/javascript">
    ...
    function changeCouponUseType() {
        if ($('input:radio[name="couponUseType"]:checked').val() == 'product') {
            ...
            $('#divCouponUsePeriodTypePeriodDay').show();

        } else if ($('input:radio[name="couponUseType"]:checked').val() == 'order' || $('input:radio[name="couponUseType"]:checked').val() == 'code') {
            // 발급방식
            $('input:radio[name="couponSaveType"]:eq(0)').prop("disabled", false);
            $('input:radio[name="couponSaveType"]:eq(1)').prop("disabled", false);
            ...
            // 코드쿠폰일경우
            var thisType = $('input:radio[name="couponUseType"]:checked').val();
            if(thisType == 'code'){
                $('input:radio[name=couponKindType]:eq(0)').prop('checked', true);
                $('#labelCouponKindTypeAdd').hide();
                $('#divCouponUsePeriodTypePeriodDay').hide();
                $('.codeType').show();
            }
