...
<script type="text/javascript" xmlns="http://www.w3.org/1999/html">
    ...
    $(document).ready(function () {
        ...
        $("#frmGoods").validate({
            submitHandler: function (form) {
                ...
                oEditors.getById["testeditor"].exec("UPDATE_CONTENTS_FIELD", []);	// 에디터의 내용이 textarea에 적용됩니다.
    ...
    function setRelationGoods(frmData) {
        ...
        var cnt = $('#relationGoodsInfo input[name*=\'itemGoodsNo\']').length;
...
<form id="frmGoods" name="frmGoods" action="./goods_ps.php" method="post" enctype="multipart/form-data">
    ...
    <?php if($data['goodsNo']=="1000000000"){?>
        <script type="text/javascript" src="../admin/script/sojaeji.js"></script>
        <div class="table-title gd-help-manual" id="storeadd">
            매장추가
            <span class="depth-toggle"><button type="button" class="btn btn-sm btn-link bold depth-toggle-button" depth-name="storeadd"><span>닫힘</span></button></span>
        </div>
        <input type="hidden" id="depth-toggle-hidden-storeadd" value="<?=$toggle['storeadd_'.$SessScmNo]?>">
        <div id="depth-toggle-line-storeadd" class="depth-toggle-line display-none"></div>
        <div id="depth-toggle-layer-storeadd">
            <iframe src="/map_api.php" width='100%' height='330'frameborder='0'></iframe>
        </div>
    <?php } ?>
    <!--/--2019-12-24 매장정보추가 끝-->
    ...
    <div id="depth-toggle-layer-colorImg">
        <table class="table table-cols">
            <colgroup>
                <col class="width-lg"/>
                <col/>
            ...
            <tr>
                <th>상세 컬러, 이미지</th>
                <td class="input_area wibGoodsScrollList">
                    
                    
                    <table class="table table-rows table-fixed mgt10" id="wibGoodsScrollList" <?php if(!$data['scrollSno']){ ?>style="display:none;"<?php }?>>
                        <thead>
                            <tr>
                                <th>그룹명</th>
                                <th>선택된 컬러</th>
                                <th>선택된 이미지</th>
                                <th class="width10p">연결 해제</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php 
                                if($scrollData){
                                    foreach ($scrollData as $wKey => $wValue){
                            ?>
                            <tr id="wibGoodsScrollList<?= $wValue['sno']; ?>">
                                <td class="center">
                                    <?= $wValue['scrollName']; ?>
                                    <input type="hidden" name="scrollSno[]" value="<?= $wValue['sno']; ?>" class="form-control" readonly="readonly">
                                </td>
                                <td class="center">
                                    <span style="display: inline-block; width: 30px; height: 30px; vertical-align: middle; border: 1px solid <?=$wValue['scrollColor']; ?>; background: <?=$wValue['scrollColor']; ?>; border-radius: 400px;"></span>
                                </td>
                                <td class="center">
                                    <img src="/data/goods/<?= $wValue['scrollImg']; ?>" width="40;">
                                </td>
                                <td class="center">
                    ...
                    <label>
                        <button type="button" class="btn btn-sm btn-gray" onclick="layer_goods_scroll()">상세 컬러, 이미지 선택</button>
                    </label>
                    <p class="notice-info">
                        컬러, 이미지는 스크롤시 사용되며 최대 6개 까지 등록 됩니다.
                    </p>
                    <p class="notice-info">
                        상세 컬러, 이미지 그룹 관리는 <a href="./goods_scroll_list.php" target="_blank" class="btn-link">상품 > 상품 노출형태 관리 > 상세 컬러, 이미지 스크롤 관리</a>에서 가능합니다.
                    </p>
                </td>
            </tr>
        </table>
	...
	<div class="table-title">
        PDF 파일첨부
        <span class="depth-toggle"><button type="button" class="btn btn-sm btn-link bold depth-toggle-button" depth-name="uploadPdf"><span>닫힘</span></button></span>
    </div>
	<input type="hidden" id="depth-toggle-hidden-uploadPdf" value="<?=$toggle['uploadPdf_'.$SessScmNo]?>">
    <div id="depth-toggle-line-uploadPdf" class="depth-toggle-line display-none"></div>
    <div id="depth-toggle-layer-uploadPdf">
        <table class="table table-cols">
            <colgroup>
                <col class="width-lg"/>
                <col/>
            ...
            <tr>
                ...
                <td>
                    <div class="form-inline">
						<input type="file" name="pdfFile" class="form-control width80p" id="pdf-file" tabindex="-1" style="position: absolute; clip: rect(0px, 0px, 0px, 0px);">
                    </div>
                </td>
            </tr>
        </table>
        ...
        <table class="table table-cols">
            ...
            <tr>
                ...
                <td>
                    ...
                    <div class="mgt15 select-kcmark">
                        <hr class="select-kcmark">
                        <label class="select-kcmark <?= $display ?>">
                            <?= gd_select_box('kcmarkDivFl', 'kcmarkInfo[kcmarkDivFl]', $kcmarkDivFl, null, $data['kcmarkInfo']['kcmarkDivFl'], '선택'); ?>
                        </label>
                        <label class="select-kcmark <?= $display ?>">
                            <input type="text" name="kcmarkInfo[kcmarkNo]" class="form-control width-xl" value="<?=$data['kcmarkInfo']['kcmarkNo']?>" placeholder="인증번호 입력 시, - 포함하여 입력하세요." maxlength="20">
                        </label>
                        <div class="notice-info select-kcmark <?= $display ?>">
                            ...
                            <a class="btn-link-underline"  href="http://www.ats.go.kr/content.do?cmsid=13&cid=20174&mode=view
" target="_blank" >전기용품 및 생활용품 안전관리법 가이드라인</a>의 내용을 확인해 주세요.
            ...
            <tr>
                ...
                <td colspan="3">
                    ...
                    <div class="notice-info">
                        ...
                        도서공연비 소득공제 상품은 일반상품과 네이버 페이로 함께 결제가 불가합니다. (네이버 페이 정책 참조)
                    </div>
                </td>
            </tr>
            <tr>
                ...
                <td colspan="3">
                    ...
                    <div class="form-inline">
                        <label title="최대구매 수량을 정할 경우 선택 후 수량을 입력하세요!" class="radio-inline">
                            ...
                            <?= gd_select_box('fixedOrderCnt', 'fixedOrderCnt', $fixedSales, null, $data['fixedOrderCnt']); ?>
                            최소 구매 수량 :
                        </label>
        ...
        <div>
            ...
            <div id="goodsImageImg" class="display-none2 goods-img">
                <table class="table table-cols">
                    ...
                    <tr><!--2020-01-01 썸네일 영상추가 시작-->
                        <th>Vimeo / Youtube 영상</th>
                        <td> <div style="float: left;"> <input type="hidden" id="depth-toggle-hidden-externalVimeo" value="<?=$toggle['externalVimeo_'.$SessScmNo]?>"> <input type="text" name="externalVimeoUrl" value="<?=$data['externalVimeoUrl']; ?>" class="form-control width-2xl"> </div> <div style="float: left; padding-left: 15px;"> Vimeo Or Youtube 영상링크 입력하세요. </div> </td>
                    </tr><!--2020-01-01 썸네일 영상추가 끝-->
                    <tr>
                        <th>개별이미지</th>
                        <td>
                            <table class="table table-cols">
                    ...
                    if ($data['imageStorage'] == 'url') {
                        ...
                        if (strtolower(substr($iVal['imageName'],0,4)) =='http' ) {
                           ...
                           // else  $imageNo = $iVal['imageNo'];
                            ...
                            $preViewImg = $imageInput . gd_html_preview_image($iVal['imageName'], $data['imagePath'], $data['imageStorage'], 25, 'goods', null, null, true, true, $imageSize) .$delHtml.'&nbsp;&nbsp;'. gd_htmlspecialchars_stripslashes(gd_htmlspecialchars_slashes($iVal['imageName'], 'add'));
                            echo '	$(\'#image' . ucfirst($iVal['imageKind']) . 'PreView' . $imageNo . '\').html(\' ' . gd_htmlspecialchars_slashes($preViewImg, 'add') . ' : 가로 ' . $imageWidth . 'pixel(픽셀)&nbsp;&nbsp;\');' . chr(10);
                        }
                        $imageNo++;
                    }
    ...
    <div id="depth-toggle-layer-delivery">
        <table class="table table-cols">
            ...
            <tr>
                <th>묶음배송<?= $data['deliveryBundleSno']; ?></th>
                <td>
                    <span><input type="radio" class="wib-trigger delivery-bundle-y" name="goodsDeliveryBundle" value="y" <?php if($data['goodsDeliveryBundle'] == 'y') echo 'checked'; ?>> 가능</span>
                    <span><input type="radio" class="delivery-bundle-n" name="goodsDeliveryBundle" value="n" <?php if($data['goodsDeliveryBundle'] != 'y') echo 'checked'; ?>> 불가(개별계산)</span>
                    <label>
                        <button type="button" class="btn btn-sm btn-gray js-bundle-delivery">배송비 묶음 그룹 선택</button>
                    </label>
                    <span>
                        <input type="hidden" name="deliveryBundleOriSno" value="<?= $data['goodsDeliveryKey']; ?>">
                        <input type="hidden" name="deliveryBundleSno" value="<?= $data['goodsDeliveryKey']; ?>"/>
                        <b>선택된 배송묶음 :</b> <span class="deliveryBundleNm"><?= $data['scmBundleMethod']; ?></span>
                    </span>
                </td>
            </tr>
        </table>
                ...
                <td>
                    ...
                    <table class="table table-rows table-fixed" id="relationGoodsInfo">
                        ...
                        if ($data['relationFl'] == 'm' && is_array($data['relationGoodsNo'])) {
                            $cnt = count($data['relationGoodsNo']);
    ...
    <div class="desc_box">
		...
		<div id="textareaDescriptionShop">
			<textarea name="memo2" rows="3" style="width:100%; height:400px;" id="testeditor" class="form-control"><?=$data['memo2']; ?></textarea>
		</div>
...
<script type="text/javascript">
    //상세 컬러, 이미지 레이어
    function layer_goods_scroll(){

        $.get('layer_goods_scroll_list.php', function (data) {
            var layerForm = '<div id="layerGoodsScrollList">' + data + '</div>';
//            var layerForm = data;
            BootstrapDialog.show({
                title: '상세 컬러, 이미지 그룹',
                message: $(layerForm),
    ...
    function _wibChecks()
    {
        ...
        var _deliveryBundleSno = $('input[name="deliveryBundleSno"]').val();

        if(!_deliveryBundleSno){
            
            $('.delivery-bundle-n').trigger('click');
    ...
    $(function(){
        ...
        $('.delivery-bundle-n').on('click',function(){
            $('input[name="deliveryBundleSno"]').val('');
            $('.deliveryBundleNm').html('');
        });
        
        //묶음배송설정
        $('.wib-trigger').click(function(){

            $.ajax({
                url:'./layer_goods_delivery_bundle.php',
                type: 'post',
    ...
    function setDeliveryBundle(data){
        
        $('input[name="deliveryBundleSno"]').val(data.snoId);
        $('.deliveryBundleNm').html(data.deliveryNm);
        
        if(!($('input[name="deliveryBundleSno"]').val())){
            $('.delivery-bundle-y').attr('checked',false);
            $('.delivery-bundle-n').attr('checked',true);
        }else{
            $('.delivery-bundle-y').attr('checked',true);
            $('.delivery-bundle-n').attr('checked',false);
        }
    }
    //추가끝
