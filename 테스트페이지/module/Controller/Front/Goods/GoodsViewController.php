...
//        print_r($goodsView['goodsDescription']);
        
        $sql = "SELECT yVideoLinks FROM es_goods WHERE goodsNo = {$goodsView['goodsNo']}";
        $yVideoLinks = $this->db->fetch($sql);
        $goodsView['yVideoLinks'] = explode('^|^',$yVideoLinks['yVideoLinks']);
        
        $goodsView['scrollSno'] = explode('^|^', $goodsView['scrollSno']);
        
        $whereData = '';
        
        foreach ($goodsView['scrollSno'] as $val){
            $whereData .= $val.' or sno = ';
        }
        $whereData = substr($whereData, 0, -9);
        
        $strQuery = "SELECT scrollColor, scrollImg, sno FROM es_goodsScroll WHERE sno = {$whereData}";
        $scrollData = $this->db->query_fetch($strQuery);
        

        
        if($scrollData){
            foreach ($scrollData as $k => $val){
                $scrollData[$k]['scrollImg'] = '/data/goods/'.$val['scrollImg'];
            }
    //        print_r($scrollData);
            $_html = '';
            foreach ($scrollData as $k => $val) {
                $_html .= '<a href="#wibGoodsScroll'.$val["sno"].'">';
                $_html .= '<span style="display: inline-block; width: 30px; height: 30px; vertical-align: middle; border: 1px solid '.$val["scrollColor"].'; background: '.$val["scrollColor"].'; border-radius: 400px;"></span>';
                $_html .= '</a>';
            }
            $goodsView['goodsDescription'] = str_replace("{scroll_name}", $_html, $goodsView['goodsDescription']);
        ...
        $strSQL = "SELECT externalVimeoFl,goodsCd ,goodsNo ,externalVimeoUrl,goodsSpecific FROM es_goods WHERE goodsNo = '$goodsNo' ";
        $result = $this->db->query($strSQL);
        $data = $this->db->fetch($result);

        $goodsView['goodsSpecific'] = $data['goodsSpecific'];
        ...
        //소수점 버리기
        $goodsSpeedPt = floor($ptresult[0]['goodsSpeedPt']);
        $goodsQualityPt = floor($ptresult[0]['goodsQualityPt']);
        $goodsStatePt = floor($ptresult[0]['goodsStatePt']);
        $goodsColorPt = floor($ptresult[0]['goodsColorPt']);
        $goodsMaterialPt = floor($ptresult[0]['goodsMaterialPt']);
...
//            print_r($goodsView['brandCd']);exit;
